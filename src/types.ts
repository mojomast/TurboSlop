...
export const DesignSpec = z.object({
  ...
  /**
   * The layout blueprint this page was rendered from. Persisted so an export can
   * be reproduced or migrated, and so the structure is auditable after the fact.
   */
  blueprint: z.string().default('statement-display'),
  /** Seed for the direction selection, so a run can be reproduced exactly. */
  seed: z.number().default(0),
  /** Why this direction was chosen, and how far it sits from its siblings. */
  direction: z
    .object({
      fit: z.number(),
      novelty: z.number(),
      rationale: z.string(),
      /** Sibling directions offered alongside this one. */
      alternatives: z.array(z.object({ blueprint: z.string(), fit: z.number() })).default([]),
    })
    .optional(),
  meta: z.object({
    decider: DeciderKind,
